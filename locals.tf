...
locals {
  ...
  /*| a | b | (a: enable default kms, b: use custom kms)
    ...
    | 1 | 0 | use default kms
    | 1 | 1 | use custom kms */
  cloudwatch_log_group_kms_key_arn = length(var.cloudwatch_log_kms_key_arn) > 0 ? var.cloudwatch_log_kms_key_arn : var.is_create_default_kms ? module.cloudwatch_log_group_kms[0].key_arn : null
